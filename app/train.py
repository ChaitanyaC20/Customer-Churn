...
def train_and_log(X, y, random_state=75):
    ...
    pipe = Pipeline([('scaler', StandardScaler()), ('xg', XGBClassifier(use_label_encoder=False, eval_metric='mlogloss',
            random_state=random_state, scale_pos_weight=scale_pos_weight, verbosity=0))])

    param_grid = {
        'xg__n_estimators': [50, 75, 100], 'xg__learning_rate': [0.05, 0.1, 0.2], 'xg__max_depth': [3, 4, 5],
        'xg__colsample_bytree': [0.3, 0.6, 0.9], 'xg__subsample': [0.3, 0.6, 1.0], 'xg__reg_lambda': [1, 3, 5],
        'xg__reg_alpha': [0, 1, 2]}

    with mlflow.start_run(run_name="XGB_Churn_Model") as run:
        grid = GridSearchCV(pipe, param_grid=param_grid, cv=5, scoring='roc_auc_ovr_weighted', n_jobs=-1)
        ...
        y_test_pred = (y_test_probs >= 0.35).astype(int)
        ...
        importance.nlargest(15).plot(kind='barh', color='steelblue')
