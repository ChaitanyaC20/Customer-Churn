...
def preprocess_regresion(df: pd.DataFrame):
    ...
    "Total_Extra_Data_Charges", "Total_Refunds", "Total_Charges", "Monthly_Charge", "Avg_Monthly_GB_Download",
    "Dependents", "Referred_a_Friend", "Under_30", "Internet_Type", "Online_Backup", "Device_Protection_Plan",
    "Payment_Method", "Population", "Latitude", "Longitude", "Premium_Tech_Support", "Churn_Value",
    "Gender", "Phone_Service", "Unlimited_Data", "Avg_Monthly_Long_Distance_Charges"
]
    df = df.drop(columns=[c for c in columns_to_exclude if c in df.columns], errors='ignore')
    X = df.drop(columns=["Total_Revenue"], errors='ignore')
    y = df["Total_Revenue"]
